...
(define (select-album-date album-title d lot)
  (local ((define (album-match track)
            (string=? (track-album track) album-title))
          (define (date-match track)
            (date>= (track-played track) d))
          (define (track-match track)
            (and (album-match track) (date-match track)))
          (define (output-format item)
            (track-name item))
          (define (format-output l)
            (map output-format l)))
    (format-output (filter track-match lot))))
...
(define (select-albums lot)
  (local ((define all-albums (select-all-album-titles/unique lot))
          (define (track-and-albums album)
            (list album (select-album album lot))))
    (map track-and-albums all-albums)))
