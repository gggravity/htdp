...
(define WORM-OVER (circle 10 "solid" "red"))
...
(define (render-game-over ws)
  (overlay/align/offset
   "right" "bottom"
   (place-image WORM-OVER (posn-x (worm-loc ws)) (posn-y (worm-loc ws)) BACKGROUND)
   -10 -10
...
(define (game-over? ws)
  (or (or (< (posn-x (worm-loc ws)) 0)
          (> (posn-x (worm-loc ws)) WIDTH))
      (or (< (posn-y (worm-loc ws)) 0)
          (> (posn-y (worm-loc ws)) HEIGHT))))
