...
(define (eval-all ex da)
  (match ex
    ...
    [(add left right) (+ (eval-all left da) (eval-all right da))]
    [(mul left right) (* (eval-all left da) (eval-all right da))]
    [(fn name parm) (local ((define b (fn-def-body (lookup-fun-def da name)))
                            (define p (fn-def-parm (lookup-fun-def da name)))
                            (define value (eval-all parm da))
                            (define plugd (subst b p value)))
                      (eval-all plugd da))]
    ))
...
(define fn_area
  (make-fn-def 'area-of-circle 'r (make-mul 'close-to-pi (make-mul 'r 'r))))
(define fn_volume
  (make-fn-def 'volume-of-10-cylinder 'r (make-mul 10 (make-fn 'area-of-circle 'r))))

(define dal (list const_pi fn_area fn_volume))

(check-expect (eval-all (make-fn 'area-of-circle 1) dal) 3.14)
(check-expect (eval-all (make-fn 'volume-of-10-cylinder 1) dal) 31.4)
...
(define (parse-sl s)
  (match s
    [(list '+ arg1 arg2) (make-add (parse arg1) (parse arg2))]
    [(list '* arg1 arg2) (make-mul (parse arg1) (parse arg2))]
    [(const-def name value) (make-const-def name (parse value))]
    [(fn-def name parm body) (make-fn name parm)]
    [_ (error "ERROR: parse-sl")]
    ))

(define (parse-atom s)
  (cond
    [(number? s) s]
    [(string? s) (error "ERROR: parse-atom")]
    [(symbol? s) s]))
...
(check-expect (interpreter (* 2 (+ 1 3)) dal) 8)
(check-expect (interpreter (* close-to-pi 2) dal) 6.28)
(check-expect (interpreter (area-of-circle 1) dal) 3.14)
(check-expect (interpreter (volume-of-10-cylinder 1) dal) 31.4)
