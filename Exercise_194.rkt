...
(define (render-polygon img p)
  (connects-dots img (cons (last p) p)))

(define (connect-dots img p x)
  (cond
    [(empty? (rest p)) (render-line img (first p) x)]
    [else (render-line
           (connect-dots img (rest p) x)
           (first p) (second p))]
    ))
...
(define (render ws)
  ...
  (connect-dots MT triangle-p (make-posn 20 10))
  ;; MT
  )
...
;; Image Polygon -> Image
;; adds an image of p to img
(define (render-polygon_2 img p)
  (render-line (connects-dots img p)
               (first p)
               (last p)))

(check-expect (render-polygon_2 MT triangle-p)
              (scene+line
               (scene+line
                (scene+line MT 20 10 20 20 "red")
                20 20 30 20 "red")
...
(check-expect (render-polygon_2 MT square-p)
              (scene+line
               (scene+line
                (scene+line
                 (scene+line MT 10 10 20 10 "red")
...
(check-expect (add-at-end (list) (make-posn 10 10)) (list (make-posn 10 10)))
(check-expect (add-at-end (list (make-posn 10 10)) (make-posn 20 20))
              (list (make-posn 10 10) (make-posn 20 20)))

;; Image Polygon -> Image
;; adds an image of p to img
(define (render-polygon_3 img p)
  (connects-dots img (add-at-end p (first p))))

(check-expect (render-polygon_3 MT triangle-p)
              (scene+line
               (scene+line
                (scene+line MT 20 10 20 20 "red")
                20 20 30 20 "red")
...
(check-expect (render-polygon_3 MT square-p)
              (scene+line
               (scene+line
                (scene+line
                 (scene+line MT 10 10 20 10 "red")
