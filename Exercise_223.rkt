...
(define (hit-ceiling? landscape)
  (if (empty? landscape) #false
      (or (= (block-y (first landscape)) 0)
          (hit-ceiling? (rest landscape)))
      ))
(define (end? t)
  (hit-ceiling? (tetris-landscape t)))
