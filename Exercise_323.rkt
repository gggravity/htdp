...
(define (search-ssn btree ssn)
  (cond
    [(equal? btree NONE) #false]
    [(equal? (node-ssn btree) ssn) (node-name btree)]
    [else
     (for/or [[walk-tree `(,(node-left btree) ,(node-right btree))]]
       (search-ssn walk-tree ssn))]
    ))

(check-expect (search-ssn ROOT 99) 'a99)
(check-expect (search-ssn ROOT 63) 'a63)
(check-expect (search-ssn ROOT 38) '#false)
