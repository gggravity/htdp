...
(define (tock ws)
  (if (eating (worm-head ws) (worm-food ws))
      (worm-eat ws)
      (worm-tick ws)
      ))
...
(define (score-text ws)
  (string-append "Score: " (number->string (worm-score ws))))

(define (render ws)
  (underlay/align/offset
   "left" "top"
   (text/font (score-text ws) 24 "lightgray" "Gill Sans" 'swiss 'normal 'bold #f)
   -5 -5
   (place-image FOOD (posn-x (worm-food ws)) (posn-y (worm-food ws))
...
(define (move-up ws)
  (if (or (string=? (worm-dir ws) "UP") (string=? (worm-dir ws) "DOWN")) ws
      (make-worm (worm-head ws) "UP" (worm-tail ws) (worm-tocks ws) (worm-food ws) (worm-score ws))))

(define (move-down ws)
  (if (or (string=? (worm-dir ws) "UP") (string=? (worm-dir ws) "DOWN")) ws
      (make-worm (worm-head ws) "DOWN" (worm-tail ws) (worm-tocks ws) (worm-food ws) (worm-score ws))))

(define (move-left ws)
  (if (or (string=? (worm-dir ws) "LEFT") (string=? (worm-dir ws) "RIGHT")) ws
      (make-worm (worm-head ws) "LEFT" (worm-tail ws) (worm-tocks ws) (worm-food ws) (worm-score ws))))

(define (move-right ws)
  (if (or (string=? (worm-dir ws) "LEFT") (string=? (worm-dir ws) "RIGHT")) ws
      (make-worm (worm-head ws) "RIGHT" (worm-tail ws) (worm-tocks ws) (worm-food ws) (worm-score ws))))
...
(define (eating head food)
  (> SIDE-LENGTH
     (inexact->exact
      (sqrt
       (+
        (sqr (- (posn-y head) (posn-y food)))
